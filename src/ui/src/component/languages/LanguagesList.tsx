...
function LanguagesList() {
	...
	const [data, setData] = useState<Page<Language> | null>(null);
	...
	const createNew = () => {
		navigate("/languages/detail/add")
	};

	const navigateToPage = useCallback(
		(p?: PagingRequest) => {
			navigate(`/languages/${PagingUtil.pagingRequestToString(p)}`);
		},
		[navigate]
	);

	const navigateToDetail = (l: Language) => {
		navigate(`/languages/detail/${l.id}`);
	}
